...
class TransfersControllerTest extends TestCase
{
     ...
     * @return CashTransfer
     */
    protected function createTransferForCurrentCompany(float $amount = 500.00): CashTransfer
    {
        $response = $this->actingAsApi($this->adminUser)->postJson('/api/transfers', [
        ...
        return CashTransfer::query()
            ->where('cash_id_from', $this->cashFrom->id)
            ->where('cash_id_to', $this->cashTo->id)
            ->where('amount', $amount)
            ->latest('id')
            ->firstOrFail();
    }
    ...
    {
        $transfer = $this->createTransferForCurrentCompany(500.00);
    ...
    {
        $current = $this->createTransferForCurrentCompany(10);
        ...
        $other = CashTransfer::query()
            ->where('cash_id_from', $otherFrom->id)
            ->where('cash_id_to', $otherTo->id)
            ->where('amount', 20)
            ->latest('id')
            ->firstOrFail();
    ...
    {
        $transfer = $this->createTransferForCurrentCompany(10);
        [$otherCompany, $otherAdmin] = $this->createCompanyWithAdminUser();
    ...
    public function test_store_transfer_rejects_cash_register_not_participating_in_transfers(): void
    {
        $this->cashFrom->update(['participates_in_transfers' => false]);
        ...
        $response->assertStatus(422);
        $response->assertJsonValidationErrors(['cash_id_from']);
    }
}
