<div class="mt-6" wire:poll.5000ms>
    ...
    <div id="templateModalBackground"
        class="fixed overflow-y-auto inset-0 bg-gray-900 bg-opacity-50 z-40 transition-opacity duration-500 {{ $showTemplateForm ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none' }}"
        wire:click="closeTemplateForm">
        <div id="templateForm"
            class="fixed top-0 overflow-y-auto right-0 w-1/3 h-full bg-white shadow-lg transform transition-transform duration-500 ease-in-out z-50 container mx-auto px-2 py-1"
            style="transform: {{ $showTemplateForm ? 'translateX(0)' : 'translateX(100%)' }};" wire:click.stop>
            <button wire:click="closeTemplateForm"
            ...
            <form>
                ...
                <div class="mb-2">
                    ...
                    <select wire:model="templateCategoryId" class="w-full p-2 border rounded">
    ...
    <div id="applyTemplateModalBackground"
        class="fixed inset-0 bg-gray-900 bg-opacity-50 z-40 transition-opacity duration-500 {{ $showApplyTemplateForm ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none' }}"
        wire:click="closeApplyTemplateForm">
        <div id="applyTemplateForm"
            ...
            style="transform: {{ $showApplyTemplateForm ? 'translateX(0)' : 'translateX(100%)' }};" wire:click.stop>
            <button wire:click="closeApplyTemplateForm"
            ...
            <div class="mb-2">
                <label class="block mb-1">Категория</label>
                <select wire:model="templateCategoryId" class="w-full p-2 border rounded" disabled>
                    <option value="">Выберите категорию</option>
                    @foreach ($filteredCategories as $category)
                        <option value="{{ $category->id }}">{{ $category->name }}</option>
                    @endforeach
