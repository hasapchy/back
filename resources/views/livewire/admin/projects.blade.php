            ...
            @foreach ($projects as $project)
                @if (Auth::user()->hasPermission('edit_projects'))
                    <tr wire:click="selectProject({{ $project->id }})"
                        class="cursor-pointer mb-2 p-2 border rounded {{ $selectedProjectId == $project->id ? 'bg-gray-200' : '' }}">
                @endif
        ...
        <div id="form"
            ...
            <div class="mb-4">
                <button wire:click="$set('activeTab', 'general')"
                    class="px-4 py-2 {{ $activeTab == 'general' ? 'bg-gray-300' : '' }}">Общее</button>
                <button wire:click="$set('activeTab', 'details')"
                    class="px-4 py-2 {{ $activeTab == 'details' ? 'bg-gray-300' : '' }}">Детали</button>
            </div>
                ...
                <div class="flex space-x-2">
                    ...
                    @if (Auth::user()->hasPermission('delete_projects'))
                        @if ($selectedProjectId)
                            <button wire:click="confirmDeleteProject({{ $selectedProjectId }})"
                    ...
                    <tbody>
                        @foreach ($projectTransactions as $transaction)
                            <tr>
                                ...
                                <td class="p-2 border border-gray-200">{{ $transaction->transaction_date }}</td>
                                <td
